...
public class AccessPolicyService {
    ...
    private static final String ROLE_ENTREPRISE = "ENTREPRISE";
    private static final String ROLE_DELEGUE = "DELEGUE";
    ...
    private static final Map<String, List<String>> DELEGUE_ALLOWED_COLUMNS = Map.ofEntries(
            ...
            Map.entry("annimation_fiches", List.of("id", "id_pharmay", "id_annimatrice", "date_annimation", "etat",
                    "personnel", "points_fort", "recommandations", "finalisation", "etatPharma", "created_at", "cancled_at")),
            ...
            Map.entry("annimation_fiches_prescriptions", List.of("id", "id_fiche", "id_prescripteur", "potentiel", "pour", "produits_concurent")),
            ...
            Map.entry("annimation_verif_prescriptions", List.of("id", "id_fiche", "id_prescripteur", "commentaire", "commentaire_annimatrice")),
            ...
            Map.entry("cl_reliquat_dlg", List.of("id", "doc", "date", "dlg", "ttc", "rel", "sens", "cl", "nomcl", "zone", "diff", "fam", "nomDlg")),
            ...
            Map.entry("data_concurant_prd", List.of("id", "idprd", "unitEch", "unitnatreceonst", "unitReel",
                    "margeErr", "canatreceonst", "careelht", "margeErrca", "t", "year")),
            Map.entry("date_jouv", List.of("id", "date_j")),
            Map.entry("demande", List.of("id", "id_user", "all_pros", "prospects", "secteur", "delegation",
                    "spec", "type", "date", "lieu", "dis", "pays", "budget_demander",
                    "budget_investi", "objectif", "labos", "prod", "commentaire",
                    "validation", "public", "validation_date", "forHow")),
            Map.entry("demande_prod", List.of("id", "id_demande", "id_prod", "qte")),
            ...
            Map.entry("edit_prospect_dmd", List.of("id", "id_pros", "nom", "prenom", "potentiel", "spec", "tel", "gsm",
                    "email", "ville", "adresse", "service", "code_postal", "phyto", "dermo", "type_fid")),
            ...
            Map.entry("grm_promotionnel", List.of("id", "id_demandeur", "observation_client", "date_remise_point",
                    "date_pro", "etat", "date_livraison")),
            Map.entry("groupe", List.of("id", "nom"))
    );

    private static final Set<DataModule> DELEGUE_ALLOWED_MODULES = Set.of(
    ...
    public boolean hasFullAccess(String role) {
        return ROLE_ADMIN.equalsIgnoreCase(role) || ROLE_ENTREPRISE.equalsIgnoreCase(role);
    }
    ...
    public boolean canAccessModule(String role, DataModule module) {
        ...
        if (ROLE_DELEGUE.equalsIgnoreCase(role)) {
            return DELEGUE_ALLOWED_MODULES.contains(module);
        }
    ...
    public Set<DataModule> getAllowedModules(String role) {
        ...
        if (ROLE_DELEGUE.equalsIgnoreCase(role)) {
            return DELEGUE_ALLOWED_MODULES;
        }
    ...
    public boolean canReadTable(String role, String tableName) {
        ...
        if (ROLE_DELEGUE.equalsIgnoreCase(role)) {
            ...
            return module != null
                    && DELEGUE_ALLOWED_MODULES.contains(module)
                    && DELEGUE_ALLOWED_COLUMNS.containsKey(normalized);
        }
    ...
    public List<String> getAllowedColumns(String role, String tableName) {
        ...
        if (ROLE_DELEGUE.equalsIgnoreCase(role)) {
            return DELEGUE_ALLOWED_COLUMNS.getOrDefault(normalized, List.of());
        }
    ...
    public List<String> getAllowedTablesForRole(String role) {
        if (hasFullAccess(role)) {
            return moduleRegistryService.getAllModules().stream()
                    .flatMap(module -> moduleRegistryService.getTablesForModule(module).stream())
                    .sorted()
                    .toList();
        }

        if (ROLE_DELEGUE.equalsIgnoreCase(role)) {
            return DELEGUE_ALLOWED_MODULES.stream()
                    .flatMap(module -> moduleRegistryService.getTablesForModule(module).stream())
                    .filter(DELEGUE_ALLOWED_COLUMNS::containsKey)
                    .sorted()
                    .toList();
        }
